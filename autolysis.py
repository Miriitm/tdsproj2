...
CONFIG = {
    ...
    "OUTPUT_DIR": os.getcwd()  # Save files in the current working directory (use os.getcwd())
}
...
def visualization(plt, file_name):
    try:
        ...
        print(f"Saving visualization to: {save_path}")  # Debugging line
...
def generate_readme(df, missing_summary, correlation_matrix, outlier_summary, clustering_summary):
    ...
    print(f"Saving README.md to: {readme_path}")  # Debugging line
    try:
...
def get_dataset_file():
    ...
    if dataset_file:
        return dataset_file
    dataset_file = input("Please provide the path to the dataset CSV file: ").strip()
    if not dataset_file:
        print("Error: Dataset file path is required.")
        sys.exit(1)
    return dataset_file
